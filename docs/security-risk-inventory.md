...
| Update integrity | Same-origin checksum and legacy content-type-only updater. | Signed update metadata/artifacts. | Alpha GitHub release updater verifies trusted GitHub URL plus sha512 from Hardened `latest.json`; full signed metadata verifier TODO. |
