...
| System updates | Kernel/rootfs updates can brick the device without rollback. | Separate signed system-update bundles with staging, backup, rollback, and boot health confirmation. | Artifact contract, GitHub channel metadata tooling, staging, guarded install, and manual rollback are implemented. Signature verification, boot-good confirmation, and automatic rollback after a bad boot are still TODO. Current application updater replaces `kvmapp`, not kernel/rootfs. |
