...
| System updates | Kernel/rootfs updates can brick the device without rollback. | Separate signed system-update bundles with staging, backup, rollback, and boot health confirmation. | Artifact contract, GitHub channel metadata tooling, staging, guarded install, generated init-time rollback script, boot watchdog rollback, manual boot-good confirmation, and manual rollback are implemented. Signature verification is still TODO. Current application updater replaces `kvmapp`, not kernel/rootfs. |
